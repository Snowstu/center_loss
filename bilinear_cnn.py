...
class BCNNManager(object):
    def __init__(self, options, path):
        ...
        if self._path['pretrained'] is not None:
            self._net = BCNN(num_classes=classes, is_all=True).to(device)
            self._net.load_state_dict(torch.load(self._path["pretained"]), strict=False)
        else:
            self._net = BCNN(classes, is_all=True).to(device)

        print(self._net)
        self._criterion = nn.CrossEntropyLoss().to(device)
        self._optimizer = torch.optim.SGD(self._net.parameters(), lr=self._options["base_lr"], momentum=0.9,
        ...
        data_loader = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms)
                       for x in ['train', 'val']}
        class_to_idx = data_loader['train'].class_to_idx
        with open("labels_index.txt", "w") as f:
            f.write(str(class_to_idx))
        self._train_loader = torch.utils.data.DataLoader(data_loader["train"], batch_size=self._options["batch_size"], shuffle=True)
        self._val_loader = torch.utils.data.DataLoader(data_loader["val"], batch_size=self._options["batch_size"], shuffle=True)
...
def main():
    ...
    path = {"data": os.path.join(project_root, "data/"),
            "pretrained":None} #os.path.join(project_root, "model", args.model)}
    for d in path:
        if d == "pretrained":
